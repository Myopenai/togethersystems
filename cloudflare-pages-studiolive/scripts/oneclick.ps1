# [.SYSTEMS.T.SYSTEMS.] One-Click Canvas: Complete Plug-and-Play Code Bundle (PowerShell)
# Portal + APIs + Robotics Pipeline Simulation

$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

Write-Host "==> One-click: TogetherSystems portal + APIs + Robotics pipeline (simulation) + downloads + publish prep" -ForegroundColor Cyan

$ROOT = $PSScriptRoot

New-Item -ItemType Directory -Force -Path "public/assets", "public/downloads", "functions/api/voucher", "functions/api/presence", "functions/api/robotics", "scripts" | Out-Null
...
'@ | Set-Content -Path "public/assets/osoto.css" -Encoding UTF8
...
'@
Set-Content -Path "public/manifest-portal.html" -Value $manifestHtml -Encoding UTF8

# 4) Downloads
"OSOTODOS Downloads — Place your build artifacts here.`nThis directory will be published by Cloudflare Pages." | Set-Content -Path "public/downloads/README.txt" -Encoding UTF8
"Beispiel — Umlaute geprüft: ÄÖÜ äöü ß. NFC-normalisiert." | Set-Content -Path "public/downloads/sample.txt" -Encoding UTF8
...
'@
Set-Content -Path "public/downloads/index.html" -Value $downloadsHtml -Encoding UTF8

# 5) Functions
"export async function onRequest(context) {`n  return new Response(JSON.stringify([`n    { id: `"v-100`", amount: 5000, redeemed: 1200, status: `"active`" },`n    { id: `"v-101`", amount: 2500, redeemed: 2500, status: `"redeemed`" }`n  ]), { headers: { `"Content-Type`": `"application/json; charset=utf-8`" }});`n}" | Set-Content -Path "functions/api/voucher/list.js" -Encoding UTF8

"export async function onRequest(context) {`n  const { searchParams } = new URL(context.request.url);`n  const id = searchParams.get(`"id`");`n  return new Response(JSON.stringify({ ok: true, id, redeemed: true }), { headers: { `"Content-Type`": `"application/json; charset=utf-8`" }});`n}" | Set-Content -Path "functions/api/voucher/redeem.js" -Encoding UTF8

"export async function onRequest(context) {`n  return new Response(JSON.stringify({ online: true, users: 42, ts: new Date().toISOString() }), { headers: { `"Content-Type`": `"application/json; charset=utf-8`" }});`n}" | Set-Content -Path "functions/api/presence/status.js" -Encoding UTF8

# 6) Robotics Functions
"export async function onRequest(context) {`n  const jobId = `"job-`" + Math.random().toString(36).slice(2, 8);`n  const trace = [`n    { step: 1, name: `"material_intake`", status: `"ok`", detail: `"Scanner: PET+PP detected; volume 0.8l`" },`n    { step: 2, name: `"mix_compute`", status: `"ok`", detail: `"Safe low-temp mix computed; no chemical change`" },`n    { step: 3, name: `"print_profile`", status: `"ok`", detail: `"Layer 0.25mm; nozzle 0.4mm; temp <= critical threshold`" },`n    { step: 4, name: `"job_enqueue`", status: `"ok`", detail: `"3D-printer job queued; robot feeder engaged`" },`n    { step: 5, name: `"qc_plan`", status: `"ok`", detail: `"Dimensional checks; surface; stress sample; emissions monitor`" }`n  ];`n  return new Response(JSON.stringify({ jobId, trace }), { headers: { `"Content-Type`": `"application/json; charset=utf-8`" }});`n}" | Set-Content -Path "functions/api/robotics/start.js" -Encoding UTF8

"export async function onRequest(context) {`n  const sample = [`n    { jobId: `"job-demo-001`", status: `"queued`", printer: `"P-01`", startedAt: null },`n    { jobId: `"job-demo-002`", status: `"printing`", printer: `"P-02`", startedAt: new Date().toISOString() }`n  ];`n  return new Response(JSON.stringify(sample), { headers: { `"Content-Type`": `"application/json; charset=utf-8`" }});`n}" | Set-Content -Path "functions/api/robotics/jobs.js" -Encoding UTF8

# 7) Config
"name = `"togethersystems`"`ncompatibility_date = `"2025-12-06`"`n[site]`nbucket = `"./public`"" | Set-Content -Path "wrangler.toml" -Encoding UTF8

'{"name":"togethersystems","private":true,"scripts":{"dev":"wrangler pages dev ./public","publish":"wrangler pages publish ./public"},"devDependencies":{"wrangler":"^3.82.0"}}' | ConvertFrom-Json | ConvertTo-Json -Depth 10 | Set-Content -Path "package.json" -Encoding UTF8

# 8) NFC normalize
node -e "const fs=require('fs'),p=require('path');function norm(f){const t=fs.readFileSync(f,'utf8').normalize('NFC');fs.writeFileSync(f,t,'utf8');}function walk(d){for(const f of fs.readdirSync(d)){const x=p.join(d,f);const s=fs.statSync(x);if(s.isDirectory())walk(x);else if(/\.(html|css|js|json|txt|toml)$/i.test(f))norm(x);}}walk('public'); walk('functions'); walk('.');console.log('NFC normalization complete.');"
...
Write-Host "   2) npm run dev   (local Pages dev with Functions)" -ForegroundColor Cyan
Write-Host "   3) npm run publish (deploy to Cloudflare Pages)" -ForegroundColor Cyan

...
function Start-RootDiscovery {
    ...
    Write-Host "ROOT DISCOVERY - VOLLSTANDIGE INVENTARISIERUNG" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    # Statistiken
    $totalSize = 0
    if ($discovery.files.Count -gt 0) {
        $sizeMeasure = $discovery.files | Measure-Object -Property size -Sum -ErrorAction SilentlyContinue
        if ($sizeMeasure) {
            $totalSize = $sizeMeasure.Sum
        }
    }
    
    $byExtension = @()
    if ($discovery.files.Count -gt 0) {
        $grouped = $discovery.files | Where-Object { $_.extension } | Group-Object -Property extension -ErrorAction SilentlyContinue
        if ($grouped) {
            $byExtension = $grouped | ForEach-Object {
                $sizeSum = 0
                $sizeMeasure = $_.Group | Measure-Object -Property size -Sum -ErrorAction SilentlyContinue
                if ($sizeMeasure) {
                    $sizeSum = $sizeMeasure.Sum
                }
                @{
                    extension = $_.Name
                    count = $_.Count
...
function Create-Patches {
    ...
    # Erkenne veranderte Einstellungen
    ...
    foreach ($file in $settingsFiles) {
        ...
        $patch = @{
            ...
            description = "Settings-Update fuer $($file.name)"
            changes = @()
            status = "pending"
        }
    ...
    foreach ($file in $configFiles) {
        ...
        $patch = @{
            ...
            target = $file.path
            targetHash = $file.hash
            description = "Config-Update fuer $($file.name)"
            changes = @()
            status = "pending"
        }

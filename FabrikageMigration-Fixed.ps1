<#
.SYNOPSIS
    Fabrikage Migration Tool - Fixed Version
.DESCRIPTION
    A robust file migration tool with improved path handling and error management.
    No external dependencies or configuration files required.
.NOTES
    Version: 1.0.1
    Fixed: Path handling and confirmation issues
#>
...
param (
    ...
    [Parameter(Mandatory = $false)]
    [string]$TargetRoot = Join-Path (Split-Path $PSScriptRoot -Parent) "Fabrikage",
...
function Start-FileMigration {
    ...
    try {
        ...
        foreach ($dir in $config.Components.Directories) {
            $sourceDir = Join-Path $SourceRoot $dir -ErrorAction SilentlyContinue
